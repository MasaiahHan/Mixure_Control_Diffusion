...
class StableDiffusionCanvasControlnetPipeline(DiffusionPipeline):
    ...
    def __call__(
            ...
            for image_index in range(len(image)):
                if not isinstance(image[image_index], list):
                    image_ = self.prepare_image(
                        ...
                        num_images_per_prompt=1,
                        device=self.device,
                        dtype=torch.float32,#self.tmp[image_index].dtype,
                        do_classifier_free_guidance=7.5,
                    )
                else:
                    image_=[]
        ...
        if mode=='txt2img':
            ...
            init_image, latent_timestep, batch_size, 1, torch.float32, self.device, generator
        ).to(self.device)
        ...
        for region in text2image_regions:
            ...
            if isinstance(region.negative_prompt,str):
                uncond_tokens = [region.negative_prompt]
            else:
                uncond_tokens = [""]
            uncond_input = self.tokenizer(
                uncond_tokens * batch_size, padding="max_length", max_length=max_length, return_tensors="pt", truncation=True,
            )
